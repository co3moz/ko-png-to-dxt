    ...
    fs.createReadStream(file)
      ...
      .on('parsed', function () {
        ...
        if (!checkSize(width) || !checkSize(height)) {
          return reject(new Error('Width or Height should be power of 2 (supported ones are 4, 8, 16, 32, 64, 128, 256, 512, 1024, 2048). file: ' + file));
        }
        ...
        if (!format || format == 'a8r8g8b8') {
          ...
          imageData = dxt.compress(data, width, height, dxt.flags.DXT1);
          formatData = Buffer.from([0x44, 0x58, 0x54, 0x31, 0x01, 0, 0, 0]);
          imageData = Buffer.from(imageData);
        } else if (format == 'dxt3') {
          imageData = dxt.compress(data, width, height, dxt.flags.DXT3);
          formatData = Buffer.from([0x44, 0x58, 0x54, 0x33, 0x01, 0, 0, 0]);
          imageData = Buffer.from(imageData);
        } else if (format == 'dxt5') {
          imageData = dxt.compress(data, width, height, dxt.flags.DXT5);
          formatData = Buffer.from([0x44, 0x58, 0x54, 0x35, 0x01, 0, 0, 0]);
          imageData = Buffer.from(imageData);
        } else {
          return reject(new Error('unknown format "' + format + '"! file:' + file));
        }
        ...
        fs.writeFile(outputLocation, outputBuffer, function (err) {
          if (err) {
            console.error('error ocurred on writing ' + outputLocation);
      ...
      .on('error', function (err) {
        console.error('error ocurred on png parsing! ' + outputLocation);
        reject(err);
      });
  });
}
